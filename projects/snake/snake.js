...
async function gameOver() {
    isGameOver = true;
    const wantsToPlay = await ask('Game Over. Do you wanna play again?');
...
async function ask(question) {
    return new Promise((resolve, reject) => {
        const html = `
<p class="message__label">${question}</p>
<button class="btn btn_confirm">Yes</button>
<button class="btn btn_reject">No</button>`;
        ...
        const yes = message.querySelector('.btn_confirm');
        const no = message.querySelector('.btn_reject');
...
function eatItself() {
    ...
    for (let i = 1, length = snake.length; i < length; i++) {
        if (!(snake[i].x >= head.x + size || head.x >= snake[i].x + size)
            && !(snake[i].y >= head.y + size || head.y >= snake[i].y + size)) {
            return true;
        }
    }
